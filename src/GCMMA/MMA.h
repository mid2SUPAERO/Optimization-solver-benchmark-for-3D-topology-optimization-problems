...
class MMA{
	public:
		...
		PetscErrorCode Update(Vec xval, PetscScalar fx, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax, PetscInt cond);
		// Update [raa0, raa], build and solve GCMMA subproblem, compute [f0app, fapp]

		// Update [raa0, raa]
		PetscErrorCode RaaUpdate(Vec xmma,Vec xval, PetscScalar fxnew,PetscScalar *gxnew,Vec xmin, Vec xmax);
		
				
		PetscBool ConCheck(PetscScalar fxnew, PetscScalar *gxnew);
	...
	private:
		...
		PetscErrorCode GenSub(Vec xval, PetscScalar fx, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax, PetscInt cond);
		// Compute [f0app, fapp]
		 PetscErrorCode ComputeApprox(Vec xmma);

		// Interior point solver for the subproblem
		PetscErrorCode SolveDIP(Vec xval);

		// Solve the dual with a steepest ascent method
		PetscErrorCode SolveDSA(Vec x);
		...
		PetscScalar raa0eps,raaeps,xmamieps,epsimin;
		PetscScalar move, albefa;

		PetscScalar  r0, fapp; // residual and objective func
		PetscScalar  *r, *gapp; // residual and contraints
