...
MMA::MMA(PetscInt nn, PetscInt mm, PetscInt kk, Vec xo1t,Vec xo2t,Vec Ut,Vec Lt, 
	...
	raa  = new PetscScalar[m];
	r    = new PetscScalar[m];
	gapp = new PetscScalar[m];

	for (PetscInt i=0;i<m;i++){ raa[i] = 1.0; }
...
PetscErrorCode MMA::Update(Vec xval, PetscScalar fx, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax, PetscInt cond){
	PetscErrorCode ierr=0;
	...
	GenSub(xval,fx,dfdx,gx,dgdx,xmin,xmax,cond);
...
PetscBool MMA::ConCheck(PetscScalar fxnew, PetscScalar *gxnew){

	if (fapp + epsimin < fxnew) { return PETSC_FALSE; }
	for (PetscInt j = 0; j < m; ++j) {
		if (gapp[j] + epsimin < gxnew[j]) { return PETSC_FALSE; }
	}
	return PETSC_TRUE;
}
...
PetscErrorCode MMA::RaaUpdate(Vec xmma, Vec xval, PetscScalar fxnew,PetscScalar *gxnew,Vec xmin,Vec xmax){
	PetscErrorCode ierr=0;

	PetscScalar raacofmin = 1e-12;
	PetscScalar raacof = 0.0;
	PetscScalar xmami, xxux,xxxl,xxul,ulxx;

	PetscScalar deltaraa0,deltaraa;

	PetscInt nloc;
	VecGetLocalSize(xmma,&nloc);
	PetscScalar *xv, *xmmav, *Lv, *Uv, *xminv, *xmaxv;


	VecGetArray(xval,&xv);
	VecGetArray(xmma,&xmmav);
	VecGetArray(L,&Lv);
	VecGetArray(U,&Uv);
	...
	for (PetscInt i = 0; i < nloc; ++i) {
		xmami = Max(xmamieps, xmaxv[i] - xminv[i]);
		xxux = (xmmav[i] - xv[i]) / (Uv[i] - xmmav[i]);
		xxxl = (xmmav[i] - xv[i]) / (xmmav[i] - Lv[i]);
		xxul = xxux * xxxl;
		ulxx = (Uv[i] - Lv[i]) / xmami;
		raacof += xxul * ulxx;
	}
	raacof = Max(raacofmin, raacof);
	if (fxnew > fapp + 0.5 * epsimin) {
		deltaraa0 = (1.0 / raacof) * (fxnew - fapp);
		raa0 = Min(1.1 * (raa0 + deltaraa0), 10.0 * raa0);
	}
	for (PetscInt j = 0; j < m; ++j) {
		if (gxnew[j] > gapp[j] + 0.5 * epsimin) {
			deltaraa = (1.0 / raacof) * (gxnew[j] - gapp[j]);
			raa[j] = Min(1.1 * (raa[j] + deltaraa), 10.0 * raa[j]);
		}
	}

	
	VecRestoreArray(xval,&xv);
	VecRestoreArray(xmma,&xmmav);
...
PetscErrorCode MMA::GenSub(Vec xval, PetscScalar fx, Vec dfdx, PetscScalar *gx, Vec *dgdx, Vec xmin, Vec xmax, PetscInt cond){
	PetscErrorCode ierr=0;
		...
		if (k>2) {
			for (PetscInt i=0; i<nloc; i++){
				...
				Lv[i]=Max(Lv[i],xv[i]-100.0*xmi);
                            	Lv[i]=Min(Lv[i],xv[i]-1e-5*xmi);
                            	Uv[i]=Max(Uv[i],xv[i]+1e-5 *xmi);
	...
	for (PetscInt i=0; i<nloc; i++){
		...
		alf[i] = Max(alf[i], xv[i] - move * (xmaxv[i] - xminv[i]));
		bet[i]  = Min(xmaxv[i],0.9*Uv[i]+0.1*xv[i]);
		bet[i] = Min(bet[i], xv[i] + move * (xmaxv[i] - xminv[i]));
...
PetscErrorCode MMA::ComputeApprox(Vec xmma) {
	PetscErrorCode ierr=0;

	PetscInt nloc;
	VecGetLocalSize(xmma,&nloc);
	PetscScalar uxinv,xlinv; 

	PetscScalar *xmmav, *Lv, *Uv, *x1v, *x2v;
	PetscScalar *p0v, *q0v, **pijv, **qijv;
	...
	fapp = 0;
	for (PetscInt i = 0; i < m; ++i) { gapp[i] = 0.0; }

	for (PetscInt i = 0; i < nloc; ++i) {
		uxinv = 1.0 / (Uv[i] - xmmav[i]);
		xlinv = 1.0 / (xmmav[i] - Lv[i]);
		fapp += p0v[i] * uxinv + q0v[i] * xlinv;
		for (PetscInt j = 0; j < m; ++j) {
			gapp[j] += pijv[j][i] * uxinv + qijv[j][i] * xlinv;
		}
	}
	fapp += r0;
	for (PetscInt j = 0; j < m; ++j) {
...
PetscErrorCode MMA::SolveDSA(Vec x){
	PetscErrorCode ierr=0;

	PetscScalar tol,err,theta;
	PetscInt loop;
