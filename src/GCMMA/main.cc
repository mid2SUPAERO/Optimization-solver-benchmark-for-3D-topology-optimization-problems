...
int main(int argc, char *argv[]){
	...
	PetscBool resp = PETSC_FALSE;// Check whether the new solution is conservative
	...
	while (itr < opt->maxItr && ch > 0.01){
		...
		ierr = mma->Update(opt->x,opt->fx,opt->dfdx,opt->gx,opt->dgdx,opt->xmin,opt->xmax,1); CHKERRQ(ierr);
		...
		resp = mma->ConCheck(opt->fx,opt->gx);

		ierr = mma->RaaUpdate(opt->x,opt->xmma,opt->fx,opt->gx,opt->xmin,opt->xmax);
		ierr = mma->Update(opt->x,opt->fx,opt->dfdx,opt->gx,opt->dgdx,opt->xmin,opt->xmax,0); CHKERRQ(ierr);
