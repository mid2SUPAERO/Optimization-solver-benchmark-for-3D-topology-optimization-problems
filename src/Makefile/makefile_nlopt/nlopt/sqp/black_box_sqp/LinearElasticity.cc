...
double MyFunction_wrapper(const std::vector<double> &x, std::vector<double> &grad, void *data) {
  LinearElasticity *phys = static_cast<LinearElasticity *>(data);   
  return phys->myvfunc(x, grad, data);
}

double MyConstraint_wrapper(const std::vector<double> &x, std::vector<double> &grad, void *data) {
  LinearElasticity *phys = static_cast<LinearElasticity *>(data);   
  return phys->myvconstraint(x, grad, data);
}
...
PetscErrorCode LinearElasticity::SetUpLoadAndBC(TopOpt *opt){
        ...
        for (PetscInt i=0;i<nn;i++){
                ...
                if (i % 3 == 0 && PetscAbsScalar(lcoorp[i]-opt->xc[1]) < epsi && 
                                  PetscAbsScalar(lcoorp[i+2]-opt->xc[5]/2) < epsi){
...
double LinearElasticity::myvfunc(const std::vector<double> &x, std::vector<double> &grad, void *my_func_data){
	// Errorcode
        PetscErrorCode ierr;

	TopOpt *opt = reinterpret_cast<TopOpt*>(my_func_data);	
	PetscInt nn=opt->n;
	...
	// Loop over elements
        for (PetscInt i=0;i<nel;i++){
                ...
                // Add to objective
                fx += (opt->Emin + PetscPowScalar(x[i],opt->penal)*(opt->Emax - opt->Emin))*uKu;
                // Set the Senstivity
		if (!grad.empty()) {
                	grad[i]= -1.0 * opt->penal*PetscPowScalar(x[i],opt->penal-1)*(opt->Emax - opt->Emin)*uKu;
		}
        }
...
double LinearElasticity::myvconstraint(const std::vector<double> &x, std::vector<double> &grad, void *data){
	TopOpt *opt = reinterpret_cast<TopOpt*>(data);
   	PetscInt nn=opt->n;
...
int LinearElasticity::optimize(TopOpt *opt){
	PetscInt nn=opt->n;
	//nlopt::opt optimizer(nlopt::LD_SLSQP,nn);
	nlopt::opt optimizer(nlopt::LD_MMA,nn);
	
	
	// Initialize NLOPT OPTIMIZER
	std::vector<double> x(nn); // design variable
	std::vector<double> lb(nn); // lover bounds
	std::vector<double> ub(nn); // upper bounds
	for (PetscInt i=0;i<nn;i++){
		x[i]  = opt->volfrac;
		lb[i] = opt->Xmin;
		ub[i] = opt->Xmax;
	}
	optimizer.set_min_objective(MyFunction_wrapper, opt);
	
	
	optimizer.set_lower_bounds(lb);
	optimizer.set_upper_bounds(ub);
	optimizer.add_inequality_constraint(MyConstraint_wrapper,opt,opt->volfrac);
